...
public class FamilyService {
    ...
    public void saveData() {
        ((CollectionFamilyDao) familyDao).saveData();
    }
    ...
    public void loadData() {
        ...
        try {
            ...
            List<Family> families = mapper.readValue(file, new TypeReference<List<Family>>() {});
            familyDao.loadData(families);
            out.printf("\nLoaded %d families from file: %s\n", families.size(), DATA_FILE);
        } catch (IOException e) {
            out.printf("\nFailed to load families from file: %s, error: %s\n", DATA_FILE, e.getMessage());
        }
    }
